 ...
 */
import { REPUTATION_LIMITS, getReputationMultiplier, } from '@xache/constants';
/**
...
export function calculateReputationScore(metrics) {
    const { totalOperations, successfulOperations, failedOperations, totalContributionValue, avgResponseTime, daysSinceLastActivity, } = metrics;
    ...
    finalScore = Math.max(REPUTATION_LIMITS.MIN_REPUTATION_SCORE, Math.min(finalScore, REPUTATION_LIMITS.MAX_REPUTATION_SCORE));
    return parseFloat(finalScore.toFixed(4));
}
/**
 * Calculate reputation decay based on inactivity
 */
export function calculateReputationDecay(currentScore, daysSinceLastActivity) {
    ...
    const decayAmount = daysSinceLastActivity * REPUTATION_LIMITS.REPUTATION_DECAY_RATE_DAILY;
    const newScore = Math.max(REPUTATION_LIMITS.MIN_REPUTATION_SCORE, currentScore - decayAmount);
    return parseFloat(newScore.toFixed(4));
}
/**
 * Calculate reputation boost from contribution
 */
export function calculateContributionBoost(currentScore, contributionValueUSD) {
    ...
    const boostAmount = Math.min(contributionValueUSD / 10 * REPUTATION_LIMITS.MAX_REPUTATION_BOOST_PER_CONTRIBUTION, REPUTATION_LIMITS.MAX_REPUTATION_BOOST_PER_CONTRIBUTION);
    const newScore = Math.min(REPUTATION_LIMITS.MAX_REPUTATION_SCORE, currentScore + boostAmount);
    return parseFloat(newScore.toFixed(4));
}
/**
 ...
 */
export function updateReputationScores(current, operationSuccess, responseTimeMs) {
    const metrics = {
 ...
 */
export function calculatePriceAdjustment(basePrice, reputationScore) {
 ...
 */
export function isEligibleForDiscount(reputationScore) {
    return reputationScore < 0.4;
}
/**
 * Check if agent is in premium tier
 */
export function isPremiumTier(reputationScore) {
    return reputationScore >= 0.6;
}
/**
 * Get reputation status summary
 */
export function getReputationStatus(score) {
